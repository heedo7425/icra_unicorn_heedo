...
class ObstacleSD:
    ...
    def __init__(self, id, s_meas, d_meas, lap, size, isVisible, sector_id=-1, in_static_obs_sector=False):
        """
        ...
        # ===== HJ ADDED: Store sector information =====
        self.sector_id = sector_id
        self.in_static_obs_sector = in_static_obs_sector
        # ===== HJ ADDED END =====
...
class StaticDynamic:
    ...
    def __init__(self):
        ...
        # --- Publisher ---
        self.static_dynamic_marker_pub = rospy.Publisher('/tracking/static_dynamic_marker_pub', MarkerArray, queue_size=5)
        self.estimated_obstacles_pub = rospy.Publisher('/tracking/obstacles', ObstacleArray, queue_size=5)
        self.raw_opponent_pub = rospy.Publisher('/tracking/raw_obstacles', ObstacleArray, queue_size=5)
        if self.measuring:
            self.latency_pub = rospy.Publisher('/tracking/latency', Float32, queue_size=10)

        # --- Tunable Parameters ---
        self.rate = rospy.get_param("/perception/rate_tracking")

        Opponent_state.rate = self.rate
        Opponent_state.dt = 1/self.rate
        Opponent_state.P_vs = rospy.get_param("/perception/P_vs")
        Opponent_state.P_d  = rospy.get_param("/perception/P_d")
        Opponent_state.P_vd = rospy.get_param("/perception/P_vd")
        Opponent_state.measurment_var_s = rospy.get_param("/perception/measurment_var_s")
        Opponent_state.measurment_var_d = rospy.get_param("/perception/measurment_var_d")
        Opponent_state.measurment_var_vs = rospy.get_param("/perception/measurment_var_vs")
        Opponent_state.measurment_var_vd = rospy.get_param("/perception/measurment_var_vd")
        Opponent_state.process_var_vs = rospy.get_param("/perception/process_var_vs")
        Opponent_state.process_var_vd = rospy.get_param("/perception/process_var_vd")

        self.max_dist       = rospy.get_param("/perception/max_dist")
        self.var_pub        = rospy.get_param("/perception/var_pub")
        self.aggro_multiplier = rospy.get_param("/perception/aggro_multi")
        self.dist_deletion = rospy.get_param("/perception/dist_deletion")
        self.dist_infront = rospy.get_param("/perception/dist_infront")
        self.max_std = rospy.get_param("/perception/max_std")
        self.min_std = rospy.get_param("/perception/min_std")
        self.min_nb_meas = rospy.get_param("/perception/min_nb_meas")
        self.noMemoryMode = rospy.get_param("/perception/noMemoryMode")
        self.debug_mode = rospy.get_param("/perception/debug_mode")
        self.publish_static = rospy.get_param("/perception/publish_static")
        self.ratio_to_glob_path = rospy.get_param("/perception/ratio_to_glob_path")
        self.publish_static = rospy.get_param("/perception/publish_static")
        self.ttl_dynamic = rospy.get_param("/perception/ttl_dynamic")
        self.ttl_static = rospy.get_param("/perception/ttl_static")
        self.ttl_static_sector = rospy.get_param("/perception/ttl_static_sector", 3)
        self.vs_reset = rospy.get_param("/perception/vs_reset")
    ...
    def dyn_param_cb(self, params: Config):
        self.ttl_dynamic = rospy.get_param("dyn_perception/ttl_dynamic")
        self.ratio_to_glob_path = rospy.get_param("dyn_perception/ratio_to_glob_path")
        self.ttl_static = rospy.get_param("dyn_perception/ttl_static")
        self.ttl_static_sector = rospy.get_param("dyn_perception/ttl_static_sector", 3)
        self.min_nb_meas = rospy.get_param("dyn_perception/min_nb_meas")
        self.dist_deletion = rospy.get_param("dyn_perception/dist_deletion")
        self.dist_infront = rospy.get_param("dyn_perception/dist_infront")
        self.min_std = rospy.get_param("dyn_perception/min_std")
        self.max_std = rospy.get_param("dyn_perception/max_std")
        self.vs_reset = rospy.get_param("dyn_perception/vs_reset")
        self.aggro_multiplier = rospy.get_param("dyn_perception/aggro_multi")
        self.debug_mode = rospy.get_param("dyn_perception/debug_mode")
        self.publish_static = rospy.get_param("dyn_perception/publish_static")
        self.noMemoryMode = rospy.get_param("dyn_perception/noMemoryMode")
    ...
    def update_tracked_obstacle(self, tracked_obstacle: ObstacleSD, meas_obstacle):
        ...
        # ===== HJ MODIFIED: Use longer TTL for obstacles in static sector =====
        # Update sector information from latest measurement
        tracked_obstacle.sector_id = meas_obstacle.sector_id
        tracked_obstacle.in_static_obs_sector = meas_obstacle.in_static_obs_sector

        # Use different TTL based on whether obstacle is in smart static sector
        if tracked_obstacle.in_static_obs_sector:
            tracked_obstacle.ttl = self.ttl_static_sector  # Long TTL (e.g., 600 = 15 sec @ 40Hz)
        else:
            tracked_obstacle.ttl = ObstacleSD.ttl  # Normal TTL (e.g., 3)
        # ===== HJ MODIFIED END =====
    ...
    def update(self):
        ...
        for tracked_obstacle in self.tracked_obstacles:
            ...
            if isTracked:
                ...
                if tracked_obstacle.ttl <= 0:
                    ...
                    # ===== HJ MODIFIED: Smart static sector obstacles decrease TTL rapidly when not detected =====
                    if(tracked_obstacle.staticFlag and tracked_obstacle.in_static_obs_sector):
                        # Smart static sector: decrease TTL very rapidly when not detected
                        tracked_obstacle.ttl -= 1  # 600 ttl
                        tracked_obstacle.isVisible = False
                    
                    elif(tracked_obstacle.staticFlag and self.noMemoryMode):
                        tracked_obstacle.ttl -= 1
                    # ===== HJ MODIFIED END =====
        ...
        for meas_obstacle in meas_obstacles_copy:
            ...
            # ===== HJ EDITED: Pass sector information from detect =====
            self.tracked_obstacles.append(ObstacleSD(
                ...
                isVisible = True,
                sector_id = meas_obstacle.sector_id,
                in_static_obs_sector = meas_obstacle.in_static_obs_sector
            ))
            self.current_id += 1
    ...
    def publishObstacles (self):
        ...
        for obs in self.tracked_obstacles:
            ...
            # ===== HJ ADDED: Copy sector information =====
            obs_msg.sector_id = obs.sector_id
            obs_msg.in_static_obs_sector = obs.in_static_obs_sector
            # ===== HJ ADDED END =====
            ...
            if obs.staticFlag is None:
                ...
                if obs.dynamic_state.isInitialised:
                    if obs.dynamic_state.dynamic_kf.P[0][0]<self.var_pub:
                        ...
                        obs_msg.vs    = np.mean(obs.dynamic_state.vs_filt)
                        obs_msg.vd    = np.mean(obs.dynamic_state.vd_filt)
    ...
    def main (self):
        ...
        rospy.wait_for_message('/detect/raw_obstacles', ObstacleArray)
