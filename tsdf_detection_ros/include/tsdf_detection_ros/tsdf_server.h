...
#include "tsdf_detection_ros/mesh_vis.h"
#include "tsdf_detection_ros/ptcloud_vis.h"
#include "tsdf_detection_ros/transformer.h"
...
class TsdfServer {
 public:
  ...
  void insertPriorMapPointcloud(
      const sensor_msgs::PointCloud2::Ptr& pointcloud);

  /// Callback for prior map → ever-free initialization (one-shot).
  void priorMapEverFreeCallback(
  ...
  /**
   * Publish a RGB pointcloud of every observed voxel coloured by its
   * temporal free-space confidence:
   *   Blue  (0,100,255) — high-confidence free  (ever_free == true)
   *   Cyan  (0,200,180) — low-confidence free   (observed free, not yet ever-free)
   *   Red   (220,60,60) — occupied              (distance < occupancy threshold)
   */
  void publishEverFreeConfidencePointcloud();

  /**
   * Project the confidence classification onto a 2-D OccupancyGrid (top-down).
   * Grid values:
   *    0  — high-confidence free  (ever_free == true)
   *   50  — low-confidence free   (observed free, burn-in incomplete)
   *  100  — occupied              (distance < occupancy threshold)
   *   -1  — unknown / unobserved
   */
  void publishEverFreeConfidence2DGrid();

  /// Single Z-slice versions (use slice_level_).
  void publishEverFreeConfidenceSlice();
  void publishEverFreeConfidenceSlice2DGrid();
 ...
 protected:
  ...
  ros::Publisher confidence_pointcloud_pub_;
  ros::Publisher confidence_grid_pub_;
  ros::Publisher confidence_slice_pub_;
  ros::Publisher confidence_slice_grid_pub_;
  ros::Publisher dynamic_pointcloud_pub_;
  ros::Publisher target_pointcloud_pub_;
  ros::Publisher obstacle_pointcloud_pub_;
  ros::Publisher ground_pointcloud_pub_;

  /// Evaluation metric publishers (dynablox-style).
  ros::Publisher tp_pointcloud_pub_;   // True Positive (green)
  ros::Publisher tn_pointcloud_pub_;   // True Negative (black)
  ros::Publisher fp_pointcloud_pub_;   // False Positive (blue)
  ros::Publisher fn_pointcloud_pub_;   // False Negative (red)
  ros::Publisher oor_pointcloud_pub_;  // Out-of-range (gray)
  ...
  double slice_level_;
  bool slice_relative_to_robot_ = false;
  double current_sensor_z_ = 0.0;

  /// Input pointcloud Z filter (world frame). Points outside are discarded.
  double pointcloud_min_z_ = -std::numeric_limits<double>::max();
  double pointcloud_max_z_ =  std::numeric_limits<double>::max();


  /// Confidence visualization on/off per category.
  bool vis_high_conf_free_ = true;
  bool vis_low_conf_free_  = true;
  bool vis_occupied_        = true;

  /// Evaluation max range — points beyond this are classified as out-of-range.
  double eval_max_range_ = 100.0;
  ...
  // Dynamic object detection — temporal ever-free tracking + segmentation.
  std::unique_ptr<dynamic::EverFreeIntegrator> ever_free_integrator_;
  std::unique_ptr<dynamic::DynamicSegmenter> dynamic_segmenter_;
  std::unique_ptr<dynamic::Tracking> tracker_;
  std::unique_ptr<dynamic::DbscanClusterer> dbscan_clusterer_;
  std::unique_ptr<dynamic::UkfTracker> ukf_tracker_;
  bool enable_dynamic_segmenter_ = true;
  bool use_point_based_pipeline_ = false;
  int frame_counter_ = 0;
  ros::Time last_track_time_;

  // Patchwork++ ground segmentation.
  std::unique_ptr<patchwork::PatchWorkpp> ground_segmenter_;
  bool enable_ground_segmentation_ = false;
  bool enable_patchwork_voxelization_ = false;
  double patchwork_voxel_size_ = 0.05;

  /// Per-point cluster ID from last frame (for color-coded tracking vis).
  std::vector<int> last_point_cluster_id_;
  ...
  // Prior map ever-free initialization.
  bool enable_prior_map_ever_free_ = false;
  bool prior_map_ever_free_loaded_ = false;
  ros::Subscriber prior_map_ever_free_sub_;
  LongIndexSet prior_occupied_voxels_;
