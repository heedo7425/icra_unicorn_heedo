...
Transformer::Transformer(const ros::NodeHandle& nh,
  ...
  if (!use_tf_transforms_) {
    ...
    odom_sub_ =
        nh_.subscribe("odom_transform", 100, &Transformer::odomCallback, this);
...
void Transformer::odomCallback(const nav_msgs::Odometry& odom_msg) {
  ...
  transform_queue_.push_back(tf_msg);
}
