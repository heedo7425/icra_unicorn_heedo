...
# parse arguments
parser = argparse.ArgumentParser(description='Generate GG diagrams for a vehicle')
...
def calc_gg_points(V, g_force, alpha_list):
    # variables with scaling factors (matched to RC car physical quantities)
    mu_x_max = tire_params["p_Dx_1"] * tire_params["lambda_mu_x"]
    a_max = mu_x_max * g_earth  # ~15.7 m/s²
    ...
    N_fl_s = N_fr_s = N_rl_s = N_rr_s = tire_params["N_0"] * 4  # ~34.4 N
    N_fl = N_fl_n * N_fl_s
    N_fr = N_fr_n * N_fr_s
    N_rl = N_rl_n * N_rl_s
    N_rr = N_rr_n * N_rr_s
    F_x_n = MX.sym("F_x_n")  # total driving force
    F_x_s = vehicle_params["m"] * a_max  # ~68.8 N
    F_x = F_x_n * F_x_s
    ...
    F_x_fl_s = F_x_fr_s = F_x_rl_s = F_x_rr_s = F_x_s / 2  # ~34.4 N
    ...
    F_y_fl_s = F_y_fr_s = F_y_rl_s = F_y_rr_s = F_x_s / 2  # ~34.4 N
    ...
    # regularization to avoid 0/0 in sigma_x/sigma when both slips are near zero
    eps_sigma = 1e-6
    ...
    mu_x_max = tire_params["p_Dx_1"] * tire_params["lambda_mu_x"]
    N_ij0 = vehicle_params["m"] * g_force / 4.0  # quarter of total weight at current g
    wb = vehicle_params["a"] + vehicle_params["b"]  # wheelbase
    ...
    for alpha in alpha_list:
        # alpha-dependent initial guess: rotate (ax0, ay0) to match target direction
        rho_guess = mu_x_max * g_force * 0.3  # conservative magnitude
        ax0_alpha = rho_guess * np.sin(alpha)
        ay0_alpha = rho_guess * np.cos(alpha)
        omega_z0 = ay0_alpha / max(V, 0.5)
        delta0 = np.arctan(omega_z0 * wb / max(V, 0.5))
        Fx0 = vehicle_params["m"] * max(ax0_alpha, 0.01)
        Fy_per_wheel = vehicle_params["m"] * ay0_alpha / 4.0

        x0 = vertcat(
            ax0_alpha,
            ay0_alpha,
            V,
            ay0_alpha / max(V, 0.5) * 0.1,  # small lateral velocity
            omega_z0,
            delta0,
            N_ij0,
            N_ij0,
...
def gen_gg_polar(ax_ay_pairs, alpha_list):
    # no feasible points found for this V/g combination
    if ax_ay_pairs.ndim < 2 or ax_ay_pairs.shape[0] < 2:
        return np.zeros(len(alpha_list))
...
def calc_rho_for_V(V):
    ...
    for g_i, g_force in enumerate(g_list):
        ...
        rho_list_vehicle_frame = gen_gg_polar(
            np.column_stack((ax_points_vehicle_frame, ay_points_vehicle_frame)), alpha_list_interp
        )
        rho_vehicle_frame_tmp.append(rho_list_vehicle_frame)

        # rho velocity frame
        rho_list_velocity_frame = gen_gg_polar(
            np.column_stack((ax_points_velocity_frame, ay_points_velocity_frame)), alpha_list_interp
        )
