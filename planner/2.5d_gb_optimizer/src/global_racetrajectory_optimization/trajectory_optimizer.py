...
from vel_planner_25d.vel_planner import calc_vel_profile  ### HJ : use 2.5D vel_planner with slope support
...
def trajectory_optimizer(input_path: str,
                         ...
                         plot: bool = False,
                         plot_3d: bool = False) -> Tuple[np.ndarray, np.ndarray, np.ndarray, float]:  ### HJ : plot_3d separate from plot
    """
    ...
    if not parser.read(os.path.join(file_paths["inputs"], file_paths["veh_params_file"])):
    ...
    pars["b_ax_max_machines_file"] = json.loads(parser.get('GENERAL_OPTIONS', 'b_ax_max_machines_file'))
    ...
    if not (curv_opt_type == 'mintime' and not mintime_opts["recalc_vel_profile_by_tph"]):
        ...
        file_paths["b_ax_max_machines_file"] = os.path.join(file_paths["inputs"], "veh_dyn_info",
                                                          pars["b_ax_max_machines_file"])
    # ------------------------------------------------------------------------------------------------------------------
    ...
    # import track
    ### HJ : import_track returns 4-col [x,y,w_tr_r,w_tr_l] or 5-col [x,y,w_tr_r,w_tr_l,z] if 3D
    ...
    if not (curv_opt_type == 'mintime' and not mintime_opts["recalc_vel_profile_by_tph"]):
        ...
        _, b_ax_max_machines = tph.import_veh_dyn_info.\
            import_veh_dyn_info(ggv_import_path=file_paths["ggv_file"],
                                ax_max_machines_import_path=file_paths["b_ax_max_machines_file"])
    else:
        ggv = None
        ax_max_machines = None
        b_ax_max_machines = None
    ...
    ### HJ : use 3D create_raceline when z is available, otherwise use original 2D version
    if has_z:
        from global_racetrajectory_optimization.helper_funcs_glob.src.create_raceline_3d import create_raceline_3d

        z_resampled = reftrack_interp[:, 4]  # M points, resampled by spline_approximation

        raceline_interp, a_opt, coeffs_x_opt, coeffs_y_opt, spline_inds_opt_interp, t_vals_opt_interp, \
            s_points_opt_interp, spline_lengths_opt, el_lengths_opt_interp, z_fine, dz_ds = \
            create_raceline_3d(refline=reftrack_interp[:, :2],
                               normvectors=normvec_normalized_interp,
                               alpha=alpha_opt,
                               z=z_resampled,
                               stepsize_interp=pars["stepsize_opts"]["stepsize_interp_after_opt"])

        # slope: μ = -arcsin(dz_ds) — dz_ds is already dz/ds_3d from create_raceline_3d
        slope = -np.arcsin(np.clip(dz_ds, -1.0, 1.0))
    ...
    # ------------------------------------------------------------------------------------------------------------------
    # 3D INTERACTIVE VISUALIZATION (2.5D) ------------------------------------------------------------------------------
    # ------------------------------------------------------------------------------------------------------------------

    ### HJ : matplotlib 3D visualization — two figures:
    ###   Fig 1: raceline with actual z, track bounds, speed-colored
    ###   Fig 2: raceline with velocity as z-axis, track bounds flat
    ### HJ : matplotlib 3D visualization — separate from 2D plots, controlled by plot_3d flag
    if z_fine is not None and plot_3d:
        from mpl_toolkits.mplot3d import Axes3D

        ### HJ : bounds from reftrack_interp (M pts), centerline from reftrack_imp (N pts)
        bound_r_plt = reftrack_interp[:, :2] + normvec_normalized_interp * np.expand_dims(reftrack_interp[:, 2], 1)
        bound_l_plt = reftrack_interp[:, :2] - normvec_normalized_interp * np.expand_dims(reftrack_interp[:, 3], 1)
        z_bounds_plt = reftrack_interp[:, 4]
        z_centerline_plt = reftrack_imp[:, 4] if has_z else np.zeros(reftrack_imp.shape[0])

        # ====== Figure 1: 3D track with actual elevation + speed-colored raceline ======
        fig1 = plt.figure(figsize=(12, 8))
        ax1 = fig1.add_subplot(111, projection='3d')

        ax1.plot(reftrack_imp[:, 0], reftrack_imp[:, 1], z_centerline_plt, 'k--', linewidth=0.7, label='Centerline')
        ax1.plot(bound_r_plt[:, 0], bound_r_plt[:, 1], z_bounds_plt, color='firebrick', linewidth=1.0, label='Right bound')
        ax1.plot(bound_l_plt[:, 0], bound_l_plt[:, 1], z_bounds_plt, color='navy', linewidth=1.0, label='Left bound')

        sc1 = ax1.scatter(raceline_interp[:, 0], raceline_interp[:, 1], z_fine,
                          c=vx_profile_opt, cmap='RdYlGn', s=1)
        fig1.colorbar(sc1, ax=ax1, label='v [m/s]', shrink=0.6)
        ...
        # track bounds and centerline (flat, z=0)
        ax2.plot(reftrack_imp[:, 0], reftrack_imp[:, 1], 0.0,
                 'k--', linewidth=0.7, label='Centerline')
        ax2.plot(bound_r_plt[:, 0], bound_r_plt[:, 1], 0.0,
                 color='firebrick', linewidth=1.0, label='Right bound')
        ax2.plot(bound_l_plt[:, 0], bound_l_plt[:, 1], 0.0,
                 color='navy', linewidth=1.0, label='Left bound')

        # raceline + velocity curve on top
        ax2.plot(raceline_interp[:, 0], raceline_interp[:, 1], 0.0,
                 'r-', linewidth=0.7, label='Raceline')
        ax2.plot(raceline_interp[:, 0], raceline_interp[:, 1], vx_profile_opt,
                 'k-', linewidth=0.7, label='Velocity')

        ### HJ : vertical bars — green=accel, red=decel, gray=zero (same as result_plots.py)
        step = max(1, len(raceline_interp) // 150)
        for i in range(0, len(raceline_interp), step):
            if ax_profile_opt[i] > 0.0:
                col = 'g'
            elif ax_profile_opt[i] < 0.0:
                col = 'r'
            else:
                col = 'gray'
            ax2.plot([raceline_interp[i, 0], raceline_interp[i, 0]],
                     [raceline_interp[i, 1], raceline_interp[i, 1]],
                     [0, vx_profile_opt[i]], color=col)
    ...
    if lap_time_mat_opts["use_lap_time_mat"]:
        ...
        for i, top_speed in enumerate(top_speeds):
            for j, ggv_scale in enumerate(ggv_scales):
                ...
                vx_profile_opt = calc_vel_profile(ggv=ggv_mod,
                                                  ax_max_machines=ax_max_machines,
                                                  b_ax_max_machines=b_ax_max_machines,
                                                  v_max=top_speed,
                                                  kappa=kappa_opt,
    ...
    ### HJ : return z and slope arrays for 3D waypoint export (None if 2D track)
    ### HJ : z_bounds = centerline z at reftrack_interp positions (M pts, same count as bound1/bound2)
    z_bounds = reftrack_interp[:, 4] if has_z else None
    return traj_race_cl, bound1, bound2, t_profile_cl[-1], z_fine, slope, z_bounds
