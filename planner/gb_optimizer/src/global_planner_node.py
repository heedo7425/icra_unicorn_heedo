...
class GlobalPlanner:
    ...
    def __init__(self):
        ...
        self.input_path = os.path.join(RosPack().get_path('stack_master'), 'config', self.racecar_version)
        ...
        self.show_plots = rospy.get_param('/global_planner/show_plots', False)  # show no plots if False
        
        self.create_map = rospy.get_param('/global_planner/create_map')
        create_global_path = rospy.get_param('/global_planner/create_global_path')
        ...
        self.map_name = rospy.get_param('/global_planner/map')
    ...
    def global_plan_loop(self):
        ...
        if self.map_editor and not self.map_editor_mapping:
            ...
            # raw_dir = os.path.join(self.map_dir, self.map_name, self.map_name)
            # script_raw_dir = os.path.join(rospack.get_path('gb_optimizer'), 'scripts/finish_map_raw.sh')
            # subprocess.Popen(args=[script_raw_dir, raw_dir], shell=False)
