...
class Track3D:
    ...
    def resample(self, step_size: float):
        """Resample track data at new step_size using existing interpolators,
        then re-lock to rebuild all interpolators at the new grid.
        """
        if not self.track_locked:
            raise RuntimeError('Track must be locked before resampling.')

        total_length = self.s[-1]
        n_new = int(np.ceil(total_length / step_size)) + 1
        s_new = np.linspace(0, total_length, n_new)

        # Evaluate existing interpolators at new grid points
        x_new = np.array([float(self.x_interpolator(si)) for si in s_new])
        y_new = np.array([float(self.y_interpolator(si)) for si in s_new])
        z_new = np.array([float(self.z_interpolator(si)) for si in s_new])
        theta_new = np.array([float(self.theta_interpolator(si)) for si in s_new])
        mu_new = np.array([float(self.mu_interpolator(si)) for si in s_new])
        phi_new = np.array([float(self.phi_interpolator(si)) for si in s_new])
        w_tr_right_new = np.array([float(self.w_tr_right_interpolator(si)) for si in s_new])
        w_tr_left_new = np.array([float(self.w_tr_left_interpolator(si)) for si in s_new])
        ...
        self.__track_locked = False
        self.track_locked = True
        print(f'Resampled track: {n_new} points (step_size={step_size}m)')
    ### HJ : end
