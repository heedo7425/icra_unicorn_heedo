...
class ObstacleSpliner:
    ...
    def _process_obstacles(self):
        ...
        # 1. static_obs_in_sector: For memory tracking and GB revert decision
        # 2. all_static_obs: For do_spline (includes obstacles outside sector)
        ...
        if self.fixed_path_generated:
            ...
            # Group sector obstacles and check if ANY sector is clear (all obs >1.0m from GB)
            if static_obs_in_sector:
                sectors_obs = {}
                for obs in static_obs_in_sector:
