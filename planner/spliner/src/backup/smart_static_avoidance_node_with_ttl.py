...
class ObstacleSpliner:
    ...
    def __init__(self):
        ...
        # ===== HJ EDITED START: Smart static avoidance system =====
        ...
        # ===== HJ REMOVED: Memory system (now relying on multitracking's TTL) =====
    ...
    # ===== HJ SIMPLIFIED: Get interfering obstacles from latest_obstacles =====
    def _get_verified_obstacles(self) -> List[Tuple[int, int, float, float]]:
        """
        Get INTERFERING obstacles from latest_obstacles for path generation.

        Returns list of tuples: [(sector_id, obs_id, s, d), ...]
        Multitracking's TTL ensures these are stable.
        """
        verified_obs = []
        ...
        for obs in static_obs_in_sector:
            # Check interference with GB raceline
            if self._check_obstacle_interference(obs):
                verified_obs.append((obs.sector_id, obs.id, obs.s_center, obs.d_center))
                rospy.loginfo(f"[{self.name}] Interfering obstacle: "
                             f"sector={obs.sector_id}, id={obs.id}, s={obs.s_center:.2f}, d={obs.d_center:.2f}")

        return verified_obs
    # ===== HJ SIMPLIFIED END =====

    # ===== HJ SIMPLIFIED: Check if obstacles are ready for path generation =====
    def _check_obs_is_ready_for_path_gen(self) -> bool:
        """
        Check if we should generate fixed path.

        Conditions:
        1. 2+ sectors with static obstacles
        2. Has interfering obstacles

        Multitracking's TTL ensures stability.
        """
        static_obs_in_sector = [obs for obs in self.latest_obstacles
                                if obs.in_static_obs_sector and obs.is_static]

        # Count sectors and interfering obstacles
        sectors_with_obs = set()
        interfering_count = 0

        for obs in static_obs_in_sector:
            sectors_with_obs.add(obs.sector_id)
            if self._check_obstacle_interference(obs):
                interfering_count += 1

        # Stage 1: is_ready
        is_ready = len(sectors_with_obs) >= self.min_sectors_with_stable_obs
        ...
        if is_ready and should_generate:
            rospy.loginfo(f"[{self.name}] READY & SHOULD GENERATE! "
                         f"{len(sectors_with_obs)} sectors, {interfering_count} interfering obstacles")
            return True
        elif is_ready and not should_generate:
            rospy.loginfo_throttle(5.0,
                f"[{self.name}] Ready but no interference "
                f"({len(sectors_with_obs)} sectors, 0 interfering)")
            return False
        else:
            rospy.loginfo_throttle(5.0,
                f"[{self.name}] Waiting: {len(sectors_with_obs)}/{self.min_sectors_with_stable_obs} sectors")
            return False
    # ===== HJ SIMPLIFIED END =====

    # ===== HJ REMOVED: Position-based merge (no longer needed without memory system) =====
    ...
    def _process_obstacles(self):
        ...
        # ===== HJ SIMPLIFIED: Direct use of latest_obstacles (multitracking TTL handles persistence) =====
        # Filter obstacles into two lists:
        # 1. static_obs_in_sector: Static obstacles in smart static sector that are VISIBLE
        # 2. all_static_obs: All static obstacles (for do_spline)

        # ===== HJ MODIFIED: Filter by is_visible for immediate response =====
        static_obs_in_sector = [obs for obs in self.latest_obstacles
                                if obs.in_static_obs_sector and obs.is_static and obs.is_visible]

        all_static_obs = [obs for obs in self.latest_obstacles
                          if obs.is_static and obs.is_visible]

        # ===== HJ ADDED: Debug logging =====
        if self.fixed_path_generated:
            rospy.loginfo_throttle(1.0,
                f"[{self.name}] DEBUG: latest_obstacles={len(self.latest_obstacles)}, "
        ...
        # ===== HJ REMOVED: Memory system (now relying on multitracking's TTL) =====
        # Obstacles are now directly from latest_obstacles, persisted by multitracking's TTL

        # ===== Phase: Decide use_fixed_path and set obs_in_interest =====
        if self.fixed_path_generated:
            # POST-FIX mode: Check if we should revert to GB raceline
            # Revert if: 1) No obstacles in sector OR 2) All obstacles >1.0m from GB
            # ===== HJ SIMPLIFIED: Use static_obs_in_sector directly (multitracking TTL provides persistence) =====

            if not static_obs_in_sector:
                # No obstacles in sector → revert to GB
                if self.use_fixed_path:
                    self.use_fixed_path = False
                    rospy.loginfo_throttle(2.0, f"[{self.name}] POST-FIX: No obstacles in sector, reverting to GB")
            else:
                # Obstacles in sector → check if all are far from GB
                sectors_obs = {}
                for obs in static_obs_in_sector:
                    if obs.sector_id not in sectors_obs:
                        sectors_obs[obs.sector_id] = []
                    sectors_obs[obs.sector_id].append(obs)

                # Check each sector: if all obstacles >1.0m from GB, revert
                for sector_id, obs_list in sectors_obs.items():
                    if all(abs(obs.d_center) > self.gb_revert_threshold_m for obs in obs_list):
                        # Sector clear → revert to GB for optimal racing
                        self.use_fixed_path = False
                        rospy.loginfo_throttle(2.0,
                            f"[{self.name}] POST-FIX: Sector {sector_id} clear ({len(obs_list)} obs >"
                            f"{self.gb_revert_threshold_m}m from GB), reverting to GB")
                        break  # Found clear sector
            # ===== HJ SIMPLIFIED END =====
