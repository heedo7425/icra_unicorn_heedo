...
class Opponent_Trajectory:
    def __init__(self):
        ...
        # Subscriber
        rospy.Subscriber("/tracking/obstacles", ObstacleArray, self.obstacle_cb)
...
#########################HELPER FUNCTIONS######################################
    ...
    def _opp_to_marker(self, proj_opp: ProjOppPoint, id: int, delete_bool: bool = False) -> Marker:
        ...
        # 3D: get track surface z at s
        try:
            z_surface = float(self.converter.spline_z(float(proj_opp.s)))
        except Exception:
            z_surface = 0.0
        ...
        marker.pose.position.z = z_surface
