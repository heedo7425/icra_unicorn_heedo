...
class CarStateNode():
    def __init__(self, prop_state=False):
        ...
        self.ODOM_TOPIC = rospy.get_param('/carstate_node/odom_topic', default='/state_estimation/odom')
        self.IMU_TOPIC = rospy.get_param('/carstate_node/imu_topic', default='/imu/data')
        ...
        while not self.tf_buffer.can_transform("map", "base_link", rospy.Time(), rospy.Duration(1.0)):
            rospy.logwarn("Waiting for map->base_link transformation")
    ...
    def state_loop(self):
        ...
        while not rospy.is_shutdown():
            ...
            carstate_odom_msg = self.ekf_odom
            #Get rpy from imu
