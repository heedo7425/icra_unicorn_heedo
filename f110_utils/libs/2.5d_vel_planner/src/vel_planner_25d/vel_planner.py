...
def __solver_fb_acc_profile(p_ggv: np.ndarray,
    ...
    if backwards:
        ...
        slope_mod = np.flipud(slope)  ### HJ : flip slope for backward pass
        mode = 'decel_backw'
    else:
        radii_mod = radii
...
def calc_ax_poss(vx_start: float,
    ...
    if mode == 'accel_forw':
        ...
        # ax_avail_vehicle = b_ax_max_machines
    # ------------------------------------------------------------------------------------------------------------------
    # CONSIDER DRAG ----------------------------------------------------------------------------------------------------
    # ------------------------------------------------------------------------------------------------------------------
    ...
    if mode in ['accel_forw', 'decel_forw']:
        ax_final = ax_avail_vehicle + ax_drag + ax_gravity  ### HJ : added gravity
        # attention: this value will now be negative in forward direction if tire is entirely used for cornering
    else:
        ax_final = ax_avail_vehicle - ax_drag - ax_gravity  ### HJ : backward pass reverses external forces
