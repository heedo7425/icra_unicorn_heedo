...
class SectorSlicer:
    ...
    def sector_gui(self):
        ...
        def update_map(cur_s):
            ax1.cla()
            ax1.plot(x, y, z, 'r-', linewidth=0.7)
            ...
            ax1.set_ylabel('y [m]')
            ax1.set_zlabel('z [m]')
            ax1.set_title('Sector Slicing (idx=%d, s=%.1fm)' % (cur_s, s[cur_s]))
    ...
    def sectors_to_yaml(self):
        ...
        ### HJ : copy to 3D sector_tuner package
        ros_path = rospkg.RosPack().get_path('sector_tuner_3d')
        ...
        time.sleep(1)
        print('Building sector_tuner_3d...')
