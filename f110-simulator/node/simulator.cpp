...
/////////////////////////////////////////////////////////////////////////////
    ...
    void add_obs(int ind) {
        double obstacle_radius_m = 0.2;

        int radius_in_pixels = static_cast<int>(obstacle_radius_m / map_resolution);
        ...
        for (int i = -radius_in_pixels; i <= radius_in_pixels; i++) {
            for (int j = -radius_in_pixels; j <= radius_in_pixels; j++) {
                ...
                if (std::sqrt(i*i + j*j) <= radius_in_pixels) {
                    int current_r = rc[0] + i;
                    int current_c = rc[1] + j;

                    if (current_r >= 0 && current_r < map_height && current_c >= 0 && current_c < map_width) {
                        int current_ind = rc_2_ind(current_r, current_c);
                        current_map.data[current_ind] = 100; 
                    }
                }
