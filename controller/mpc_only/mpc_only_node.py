#!/usr/bin/env python3
"""
MPC-only controller node — global raceline direct tracking.

Unlike controller/mpc/mpc_node.py which consumes /behavior_strategy.local_wpnts
(produced by state_machine + planner_spline/recovery_spliner chain), this node:
  - Subscribes directly to /global_waypoints_scaled (velocity-scaler output)
  - Slices a sliding window around the car's current s each tick
  - Feeds that window into the same build_preview() / solve_once() pipeline

No state_machine, no behavior_strategy, no planner_spline dependency.

Obstacle avoidance would be added later as an OCP constraint (Phase 2) by
subscribing /tracking/obstacles and inserting hard/soft constraints on n
relative to each obstacle's (s, d_center). Not implemented here.

RViz topics:
  /mpc_only/prediction   MarkerArray   predicted xy over horizon (green)
  /mpc_only/reference    MarkerArray   raceline window (orange)
  /mpc_only/solve_ms     Float32       solve time per tick
"""
...
class MpcOnlyNode:
    def __init__(self) -> None:
        rospy.init_node("mpc_only_controller", anonymous=False)
        self.name = "mpc_only_controller"
        self.lock = threading.Lock()

        # ---- Params (under /mpc_only/* namespace) ----
        NS = "mpc_only"
        ...
        if not self.test_mode:
            try:
                ...
                codegen_dir = rospy.get_param(
                    f"{NS}/codegen_dir", "/tmp/mpc_only_c_generated"
                )
        ...
        self.pred_pub = rospy.Publisher("/mpc_only/prediction", MarkerArray, queue_size=1)
        self.ref_pub = rospy.Publisher("/mpc_only/reference", MarkerArray, queue_size=1)
        self.solve_time_pub = rospy.Publisher("/mpc_only/solve_ms", Float32, queue_size=1)
        ...
        # Try scaled first; raw /global_waypoints is the fallback if no velocity_scaler.
