#!/usr/bin/env python3
"""
ekf_mpc controller node — global raceline tracking with runtime μ.

Fork of controller/mpc_only/mpc_only_node.py. Adds a runtime μ input: instead
of hard-coding mu_default, the node subscribes to /ekf_mpc/mu_estimate (Float32)
...
μ source selection is done OUTSIDE this node via launch arg mu_source:
  ...
  ground_truth  — mu_patch_publisher publishes /mu_ground_truth (patch lookup)
                  remapped to /ekf_mpc/mu_estimate at launch
  rls           — mu_estimator_rls publishes /ekf_mpc/mu_estimate
  gp            — mu_estimator_gp  publishes /ekf_mpc/mu_estimate

RViz topics:
  /ekf_mpc/prediction   MarkerArray   predicted xy over horizon (green)
  /ekf_mpc/reference    MarkerArray   raceline window (orange)
  /ekf_mpc/solve_ms     Float32       solve time per tick
  /ekf_mpc/mu_used      Float32       μ value actually fed into OCP this tick
"""
...
class EkfMpcNode:
    def __init__(self) -> None:
        rospy.init_node("ekf_mpc_controller", anonymous=False)
        self.name = "ekf_mpc_controller"
        self.lock = threading.Lock()

        # ---- Params (under /ekf_mpc/* namespace) ----
        NS = "ekf_mpc"
        ...
        self.pred_pub = rospy.Publisher("/ekf_mpc/prediction", MarkerArray, queue_size=1)
        self.ref_pub = rospy.Publisher("/ekf_mpc/reference", MarkerArray, queue_size=1)
        self.solve_time_pub = rospy.Publisher("/ekf_mpc/solve_ms", Float32, queue_size=1)
        self.mu_used_pub = rospy.Publisher("/ekf_mpc/mu_used", Float32, queue_size=1)
        ...
        if self.mu_source != "static":
            mu_topic = str(rospy.get_param(f"{NS}/mu_estimate_topic", "/ekf_mpc/mu_estimate"))
        ...
        rospy.Subscriber("/ekf_mpc/mu_adapt_enable", Bool, self._mu_enable_cb, queue_size=1)
