...
Subscribes:
  /ekf_mpc/cmd_raw                — MPC 원 명령
  /mu_ground_truth               — 패치 lookup μ
  /ekf_mpc/mu_used                — MPC 가 실제 OCP 에 주입한 μ

Publishes:
  /vesc/high_level/ackermann_cmd_mux/input/nav_1
  /ekf_mpc/cmd_scaled_debug      — Float32 ratio
  /ekf_mpc/slip_indicator        — Float32 slip 강도 (0=없음, 1=심각)
"""
...
class MuApplier:
    def __init__(self) -> None:
        ...
        self.ratio_pub = rospy.Publisher("/ekf_mpc/cmd_scaled_debug", Float32, queue_size=1)
        self.slip_pub = rospy.Publisher("/ekf_mpc/slip_indicator", Float32, queue_size=1)

        rospy.Subscriber(
            rospy.get_param("~in_topic", "/ekf_mpc/cmd_raw"),
            AckermannDriveStamped, self._cmd_cb, queue_size=10,
        )
        rospy.Subscriber("/mu_ground_truth", Float32, self._gt_cb, queue_size=1)
        rospy.Subscriber("/ekf_mpc/mu_used", Float32, self._used_cb, queue_size=1)
